import { Outlet } from "react-router-dom";
import { Sidebar, MobileSidebar, Header } from "../../components/layout";
import { Toast } from "../../components/ui";
import { useUIStore, useAuthStore } from "../../store";
import { ROUTES } from "../../constants";
import {
  LayoutDashboard,
  BookOpen,
  Award,
  ShoppingCart,
  User,
  Bell,
  Settings,
} from "lucide-react";

const StudentLayout = () => {
  const { sidebarOpen, toasts, removeToast } = useUIStore();
  const { user } = useAuthStore();

  const studentNavItems = [
    { label: "Overview", icon: LayoutDashboard, path: ROUTES.DASHBOARD },
    { label: "My Courses", icon: BookOpen, path: ROUTES.MY_COURSES },
    { label: "Certificates", icon: Award, path: ROUTES.CERTIFICATES },
    { label: "Cart", icon: ShoppingCart, path: ROUTES.CART },
    { label: "Notifications", icon: Bell, path: ROUTES.NOTIFICATIONS },
    { label: "Profile", icon: User, path: ROUTES.PROFILE },
  ];

  if (user?.role === "admin") {
    studentNavItems.push({ label: "Admin Dashboard", icon: Settings, path: ROUTES.ADMIN_DASHBOARD });
  } else if (user?.role === "instructor") {
    studentNavItems.push({ label: "Instructor Dashboard", icon: Settings, path: ROUTES.INSTRUCTOR_DASHBOARD });
  }

  return (
    <div className="min-h-screen bg-gray-50 flex" dir="ltr">

      {/* ── Desktop Sidebar ────────────────────────────────────── */}
      <Sidebar navItems={studentNavItems} accentColor="blue" />

      {/* ── Mobile Sidebar Drawer ──────────────────────────────── */}
      <MobileSidebar navItems={studentNavItems} accentColor="blue" />

      {/* ── Main Area ─────────────────────────────────────────── */}
      <div
        className={[
          "flex-1 flex flex-col min-w-0 transition-all duration-300",
          sidebarOpen ? "lg:ml-64" : "lg:ml-20",
        ].join(" ")}
      >
        {/* Sticky top bar */}
        <Header />

        {/* Scrollable page content */}
        <main className="flex-1 p-4 sm:p-6 overflow-auto">
          <Outlet />
        </main>
      </div>

      {/* ── Global Toasts ──────────────────────────────────────── */}
      <div
        aria-live="polite"
        className="fixed bottom-5 right-5 z-50 flex flex-col-reverse gap-2 w-80 max-w-[calc(100vw-2.5rem)]"
      >
        {toasts.map((toast) => (
          <Toast
            key={toast.id}
            toast={toast}
            onClose={() => removeToast(toast.id)}
          />
        ))}
      </div>
    </div>
  );
};

export default StudentLayout;